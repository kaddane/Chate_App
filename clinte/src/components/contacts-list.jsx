...
function ContactsList({ contacts, isChannel = false }) {
    ...
    const { selectedChatData, setSelectedChatData, setSelectedChatType, setSelectedCateMessage } = userAppStore()
        ...
        <div className="mt-5">
            {
                contacts.map((contact, index) => (
                    <div key={index} onClick={() => handleClick(contact)} className={` pl-10 py-2 transition-all duration-300 cursor-pointer ${selectedChatData && selectedChatData._id === contact._id ? "bg-[#8417ff] hover:bg-[#8417ff]" : "hover:bg-[#f1f1f111]"} `} > 
